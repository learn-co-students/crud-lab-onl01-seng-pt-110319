...
describe('Reviews Component', () => {
  ...
  it('displays a review for when it is associated with the restaurant', () => {
    ...
    store.dispatch({ type: 'ADD_REVIEW', review: { text: "Was great", id: restaurantId } })
    store.dispatch({ type: 'ADD_REVIEW', review: { text: "Was not great", id: restaurantId } })
    const wrapper = mount(<Provider store={store}><App /></Provider>);
  ...
  it('does not display any review unassociated with the restaurant', () => {
    ...
    let restaurantId = store.getState().restaurants[0].id
    store.dispatch({ type: 'ADD_REVIEW', review: { text: "it was good", id: restaurantId } })
    store.dispatch({ type: 'ADD_REVIEW', review: { text: "it was great", id: restaurantId } })
    store.dispatch({ type: 'ADD_REVIEW', review: { text: "it was bad", id: "test"} })
    const wrapper = mount(<Provider store={store}><App /></Provider>);
    expect(wrapper.find(Review)).to.have.length(2);
